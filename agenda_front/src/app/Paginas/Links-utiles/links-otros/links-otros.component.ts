...
@Component({
  selector: 'app-links-otros',
  templateUrl: './links-otros.component.html',
  styleUrls: ['./links-otros.component.css']
})
export class LinksOtrosComponent implements OnInit {
  ...
  private obtenerLista(paginaActual: number, elementosPorPagina: number) {
    this.httpService.realizarGet(`${this.url.getLinks}Otro&pagina=${paginaActual}&cantidad=${elementosPorPagina}`).subscribe((data: any) => {
      if (data.state == 'OK') {
